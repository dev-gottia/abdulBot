...
var urls = [
  ...
  "https://streamsgate.net/dashboard/mma",
...
async function runScriptForURL(url) {
  ...
  try {
    ...
    if (browser) {
      // Close the browser only if it was successfully opened
      await browser.close();
    }
  }
}
...
async function runProgram() {
  console.log("Successfully Running the Puppeteer Script");
  ...
  try {
    ...
    console.error("Error in the program:", programError);
  }
}
...
schedule.scheduleJob("15 * * * *", async () => {
  try {
    // Run the program every hour at 15 minute past the hour
    await runProgram();
  } catch (scheduleError) {
    console.error("Error in scheduled job:", scheduleError);
  }
});
